...
export default function Shell({ title, tag, accent, blurb, children, aside }: Props) {
  return (
    <div className="min-h-full text-white/90" style={{ background: "radial-gradient(1200px 700px at 50% -10%, #16202f, #0a0d13 60%)" }}>
      <header className="flex items-center gap-4 px-5 sm:px-8 h-16 border-b border-white/8">
        ...
        <div className="flex items-baseline gap-3 min-w-0">
          <span className="font-display text-lg font-semibold tracking-tight" style={{ color: accent }}>{title}</span>
          <span className="text-[11px] tracking-[0.03em] font-medium text-white/35 hidden sm:inline">{tag}</span>
        </div>
        <div className="ml-auto text-xs text-white/40 max-w-[40%] text-right hidden md:block">{blurb}</div>
      </header>
