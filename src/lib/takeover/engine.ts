...
export function radiusOf(value: number): number {
  return 26 + 20 * Math.log10(Math.max(value, 1e6) / 1e6);
}
...
export class TakeoverRun {
  ...
  constructor(roundSeconds = 90, seed?: number, playerName?: string) {
    const day = new Date();
    const daySeed =
    ...
    this.milestoneIdx = 0;
    for (let i = 0; i < COMPANY_COUNT; i++) this.spawnCompany(true);
  ...
  // arena for round start; otherwise spawns land on the outer ring.
  private spawnCompany(everywhere = false) {
    const w = this.worth;
    const pool = COMPANIES.filter(
      (c) =>
        c.cap > w / 30 &&
        c.cap < w * 60 &&
        !this.companies.some((b) => b.c.name === c.name),
    );
    ...
    if (wantLocal) {
      ...
      // Half the listed spawns are edible, half are trouble.
      const edible = pool.filter((c) => c.cap < w);
      const scary = pool.filter((c) => c.cap >= w);
      pick =
        edible.length && (this.rng() < 0.5 || !scary.length)
  ...
  step(dt: number, cursorX: number, cursorY: number) {
    ...
    for (const blob of this.companies) {
      ...
      if (fleeing) {
        ...
        for (const other of this.companies) {
          if (other === blob || other.cap > blob.cap * 0.75) continue;
        ...
        if (playerCell && !blob.c.local && w < blob.cap * 0.75) {
    ...
    // Population upkeep: cull the far and the irrelevant, refill the ring.
