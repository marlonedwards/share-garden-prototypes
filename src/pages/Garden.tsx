...
const CROPS: Crop[] = [
  { id: "nova", name: "Tomatoes", ticker: "NOVA", sprite: "p-tomato", family: "Nightshades", sector: "Technology", kind: "growth", blurb: "You only profit when you harvest." },
  { id: "pepr", name: "Peppers", ticker: "PEPR", sprite: "p-pepper", family: "Nightshades", sector: "Technology", kind: "growth", blurb: "The same blight takes peppers and tomatoes at once." },
  { id: "volt", name: "Corn", ticker: "VOLT", sprite: "p-corn", family: "Grasses", sector: "Energy", kind: "income", blurb: "A steady staple that drops a little harvest each season." },
  { id: "iron", name: "Pumpkins", ticker: "IRON", sprite: "p-pumpkin", family: "Gourds", sector: "Industrials", kind: "income", blurb: "Sturdy, and it pays a small yield through the season." },
  { id: "cane", name: "Berries", ticker: "CANE", sprite: "p-berry", family: "Berries", sector: "Consumer", kind: "income", blurb: "Slow to establish, then a reliable picker." },
  { id: "aura", name: "Garlic", ticker: "AURA", sprite: "p-garlic", family: "Alliums", sector: "Health", kind: "income", blurb: "Holds its value when the market turns." },
];
...
export default function Garden() {
  ...
  const buy = (id: string) => {
    ...
    flash(`You bought ${fmtMoney(cost)} of ${cropOf(id).name}.`);
  ...
  const harvest = (id: string) => {
    ...
    flash(`You sold ${cropOf(id).name} for ${fmtMoney(proceeds)}.`);
  };
  ...
  return (
    <Shell title="Share Garden" accent="#7fb069"
      blurb="Go to market and plant a seed."
      aside={
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          ...
          <p className="mt-4 text-[13px] text-white/45 leading-relaxed">Everything you own grows in one bed.</p>
        </div>
      }>
      <div className="device device-landscape flex flex-col select-none" style={{ background: "linear-gradient(#bfe3ea 0%, #cfe9df 30%, #a9c47f 30%, #86a85f 100%)" }}>
        ...
        <div className="h-11 flex items-center gap-3 px-4 flex-shrink-0" style={{ background: "rgba(43,32,24,0.85)" }}>
          ...
          <div className="text-white/90 text-[13px]">Garden worth <span className="font-bold tnum text-white">{fmtMoney(nw)}</span></div>
          <span className="px-1.5 py-0.5 rounded text-[12px] font-medium tnum" style={{ background: pnl >= 0 ? "#9bd45f22" : "#ff8a9422", color: pnl >= 0 ? "#bfe89a" : "#ff8a94" }}>{fmtPct(pnl)}</span>
          <div className="ml-auto flex items-center gap-3">
            <span className="text-[12px] text-white/55 tnum hidden sm:inline">The co-op field is at {fmtPct(benchPnl)}</span>
        ...
        <div className="relative flex-1 overflow-hidden">
            ...
            {order.map((id) => {
              ...
              return (
                <button key={id} onClick={() => harvest(id)} className="absolute" title={`Harvest ${c.name}`}
                  ...
                  {g >= 1 && <div className="absolute -top-2 left-1/2 -translate-x-1/2 px-1.5 py-px rounded-full text-[12px] font-bold text-black" style={{ background: "#ffe08a" }}>ripe</div>}
                  {coin && <div className="absolute -top-4 left-1/2 -translate-x-1/2 float-coin"><span className="px-1.5 py-0.5 rounded-full text-[12px] font-bold text-[#ffcf8a] whitespace-nowrap tnum" style={{ background: "rgba(43,32,24,0.85)" }}>+{coin.amt}</span></div>}
                </button>
              );
            })}
          </div>
          ...
          <button onClick={() => (coopH ? harvest("coop") : buyCoop())} className="absolute right-[3%] bottom-[9%] group" style={{ width: "150px" }}>
            <div style={{ filter: affected("index") ? "grayscale(0.4) brightness(0.85)" : "none" }}>
              ...
              {!coopH && <div className="absolute inset-0 grid place-items-center"><span className="px-2 py-1 rounded-full text-[12px] font-semibold text-black animate-pulse" style={{ background: "#7ee0c0" }}>Buy in</span></div>}
            </div>
            <div className="text-center -mt-1">
              <div className="text-[13px] font-bold text-[#243b32]">Co-op field</div>
              {coopH ? <div className="text-[12px] font-semibold tnum" style={{ color: "#1c5c48" }}>{fmtMoney(coopH.shares * m.prices["coop"])}</div> : <div className="text-[12px] text-[#243b32]/70">It tends itself.</div>}
            </div>
          </button>

          {ev && (
            <div className="absolute top-2 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 px-3 py-1.5 rounded-full text-[12px] pop-in shadow-lg" style={{ background: "rgba(43,32,24,0.92)" }}>
          ...
          {toast && <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-30 px-3 py-1.5 rounded-full text-[12px] font-medium text-white pop-in tnum" style={{ background: "rgba(28,46,36,0.94)" }}>{toast}</div>}
        </div>

        {/* footer */}
        <div className="h-10 flex items-center gap-4 px-4 text-[12px] flex-shrink-0" style={{ background: "rgba(43,32,24,0.9)" }}>
          <span className="text-white/50">Tap a ripe crop to harvest it.</span>
          <span className="text-[#ffcf8a] font-medium">Income crops drop coins each season.</span>
          <span className="ml-auto text-white/50 tnum">{fmtMoney(m.dividendsCollected)} in dividends</span>
        </div>
        ...
        {done && (
          <div className="absolute inset-0 z-40 bg-black/70 backdrop-blur-sm grid place-items-center p-6" onClick={reset}>
            <div className="text-center pop-in">
              <div className="text-white/60 text-sm">The season is over.</div>
              <div className="text-4xl font-bold text-white tnum my-2">{fmtMoney(nw)}</div>
              <div className="tnum mb-2 text-[13px]" style={{ color: pnl >= benchPnl ? "#bfe89a" : "#ff8a94" }}>Your garden {fmtPct(pnl)}, the co-op field {fmtPct(benchPnl)}</div>
              <div className="text-[13px] text-white/55 max-w-[280px] mx-auto">{pnl >= benchPnl ? "You beat the co-op field this run." : "The co-op field you left alone out-yielded the crops you fussed over."}</div>
  ...
  function buyCoop() { const cost = Math.min(250, m.cash); if (cost < 10) { flash("Not enough coins."); return; } act((mk) => mk.buy("coop", cost)); flash("You bought into the co-op field."); if (speed === 0) setSpeed(1); }
}
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col" style={{ background: "#efe6d3" }}>
      <div className="h-11 flex items-center px-4 flex-shrink-0" style={{ background: "#2a2018" }}>
        <span className="text-[17px] font-bold text-[#f4efe1]">Market</span>
        <span className="ml-3 text-[12px] text-[#c9b892]">Seeds are bought and sold here.</span>
        <div className="ml-auto flex items-center gap-3">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-3 py-2">
        {CROPS.map((c) => {
          ...
          return (
            <div key={c.id} className="rounded-xl mb-1.5 overflow-hidden" style={{ background: "#f7f1e4", border: "1px solid #e0d4bb" }}>
              <div className="flex items-center gap-2.5 px-2.5 py-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[12px] text-[#8a7a5e] tnum">{c.ticker}</span>
                    <span className="text-[12px] px-1.5 py-px rounded-full" style={{ background: "#e6dcc4", color: "#7a6a4e" }}>{c.family}, {c.sector}</span>
                  </div>
                  <div className="text-[12px] text-[#8a7a5e]">{c.kind === "income" ? "Pays dividends." : "Grows, and pays no dividend."}{held ? ` You own ${fmtMoney(held.shares * price)}.` : ""}</div>
                </div>
                ...
                <div className="text-right w-[70px]">
                  ...
                  <div className="text-[12px] tnum" style={{ color: up ? "#3f7a3a" : "#c7502f" }}>{fmtPct(chg)}</div>
                </div>
              ...
              {open && (
                <div className="px-3 pb-2.5 pt-0.5 text-[12px] leading-relaxed text-[#5a4d3a]" style={{ background: "#f0e8d6" }}>
        ...
        <div className="text-center text-[12px] text-[#8a7a5e] py-2">Buying plants a seed in your bed.</div>
      </div>
    </div>
  );
}
